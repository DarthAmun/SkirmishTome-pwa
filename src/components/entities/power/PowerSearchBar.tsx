...
const PowerSearchBar = () => {
  ...
  const search = () => {
    ...
    setOpen(false);
    history.push(`/power-overview?filter=${JSON.stringify(newFilters)}`);
  };
  ...
  const reset = () => {
    ...
    history.push(`/power-overview`);
  };

  const createNewPower = () => {
    let newPower = new Power();
    delete newPower.id;
    createNewWithId("powers", newPower, (id) => {
      history.push(`/power-detail/id/${id}`);
    });
  };

  const exportFiltered = () => {
    exportFilteredFromTable("powers", filters, "SkirmishTome_filtered_powers.json");
  };
  ...
  return (
    <>
      ...
      <CreateButton onClick={() => createNewPower()}>
