...
const CharacterSearchBar = () => {
  ...
  const search = () => {
    ...
    setOpen(false);
    history.push(`/character-overview?filter=${JSON.stringify(newFilters)}`);
  };
  ...
  const reset = () => {
    ...
    history.push(`/character-overview`);
  };

  const createNewCharacter = () => {
    history.push(`/character-lab`);
  };
  ...
  const exportFiltered = () => {
    exportFilteredFromTable(
      "characters",
      filters,
      "SkirmishTome_filtered_characters.json"
    );
  };
